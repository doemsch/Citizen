...
# This is a library of the functions we'll need to drive *everything*
...
class parse_questions(object): # This class grinds through the text file and creates questions
    ...
    def load_json(self, input_file):
        """
        This function reads in the json file full of questions and laods them into the self.questions dictonary
        input jsonpickle of self.questions
        output: loads contents into self.questions
        """
        try:
            json_file = open(input_file, 'r').read()
            self.questions = jsonpickle.decode(json_file)

        except OSError as err:
            fatal("Unable to load json file: {0}".format(err))

        # Belt and Suspenders, but better safe than sorry.
